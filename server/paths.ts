 ...
 * and the per-OS data / config directories (named after APP.id).
 */
import { fromFileUrl } from "@std/path";
import { APP, appEnv } from "../app.config.ts";

/** Repo root (the directory holding server.ts, ui/, …). */
...
/** Per-OS data directory (library storage lives under here, see server/library.ts). */
